...
public sealed class HandleParkedUseCase
{
    ...
    public async Task HandleAsync(ParkedCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var spot = await _spots.GetByCoordinatesAsync(command.Lat, command.Lng, cancellationToken)
            ?? throw new InvalidOperationException($"No spot found at coordinates ({command.Lat}, {command.Lng}).");
